...
export function GithubStarButton({ className }: { className?: string }) {
  const { data } = useSWR(`https://api.github.com/repos/${REPO}`, fetcher, {
  ...
  return (
    ...
    >
      ...
      <span
        className={cn(
          "flex items-center gap-0.5 text-muted-foreground transition-colors group-hover:text-foreground",
          stars ? "min-w-[1.5ch]" : ""
        )}
      >
