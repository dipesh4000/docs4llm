 ...
 * navigation tools plus the docs-relative tools docs4llm inferred from the
...
function extractServerEntry(config: McpServerConfig): {
  ...
  const url =
    entry?.url ??
    `${getDocs4LlmBaseUrl()}/api/mcp/${config.projectId ?? ""}/mcp`;
  const token =
 ...
 * and proxies each `tools/call` to the docs4llm remote endpoint, so users get
...
/**
 * ${config.name} — MCP server generated by docs4llm.
 * Built on the official Model Context Protocol TypeScript SDK.
 *
 * Run:
 *   npm i @modelcontextprotocol/sdk
 *   DOCS4LLM_TOKEN="${token}" node server.mjs
 */
...
const AUTHORIZATION = process.env.DOCS4LLM_TOKEN ?? ${JSON.stringify(token)};
const TOOLS = ${tools};
