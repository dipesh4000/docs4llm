...
import {
  type DocMcpContext,
  runDocMcpTool,
} from "@/lib/docs4llm/mcp-tools-runtime";
...
export function buildDocAgentSystemPrompt(projectName: string): string {
  return `You are docs4llm's documentation agent for "${projectName}". Answer the user's question using ONLY this project's documentation, which you reach through the provided tools.
